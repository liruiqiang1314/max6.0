...
use think\Request;
use think\Url;
use app\console\model\Infolist as ThisModel;
...
class Infolist extends Base
{
	...
	public function create()
	{
		if (Request::instance()->isPOST())
		{
			...
			if (true === $result) {
                $this->success('新建成功', 'console/Infolist/index');
            } else {
                $this->error($result);
            }
		}
	...
	public function update($id)
	{
		if (Request::instance()->isPOST())
		{
			...
			if (true === $result) {
                $this->success('更新成功', 'console/Infolist/index');
            } else {
                $this->error($result);
            }
		}
	...
	public function appendarg(){
		$infoclass = db::name('infoclass')->field('id,title')->order('orderid')->select();
		return [
		   'infoclass' =>$infoclass,
		];
	}
	...
	public function delete(){
		if (Request::instance()->isPOST())
		{
			...
			if($delmodel){
			    $this->success('删除成功', 'console/Infolist/index');
			}
			else{
